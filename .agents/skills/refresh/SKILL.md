...
Also skim the other instruction files (`.agents/skills/*/SKILL.md`) for facts invalidated by the same diff — e.g. host/target lists, commit scopes, referenced script paths. Keep agent-specific framing out of AGENTS.md so it stays accurate for any tool that reads it.
