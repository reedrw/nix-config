...
- **Grouping rule**: consecutive tool calls form a batch; reasoning folds it visually (header + glance rows appear immediately, duration counts up once a second) without closing it; visible assistant text, a user message, or `agent_end` closes it. Thinking durations (live and restored, via the pi-thinking-fold fork's `__piCustomUi*` globalThis maps) surface in the batch header. Extension notifications (`ctx.ui.notify` info level) fold into the open batch via the patched `showExtensionNotify`. Restored sessions rebuild batches via `scanToolGroupsFromHistory` on `session_start`.
