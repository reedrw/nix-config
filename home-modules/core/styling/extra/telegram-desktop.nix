  ...
  config = lib.mkIf config.stylix.targets.telegram-desktop.enable {
    home.activation = with config.lib.stylix.colors; {
      tg-theme = let
        ...
        '';
        theme = pkgs.runCommand "telegram-base16.zip" {
          buildInputs = with pkgs; [
            imagemagick
            zip
          ];
        } ''
          cat << EOF > colors.tdesktop-theme
          ${themeColors}
          EOF
          magick -size 100x100 xc:#${base00} tiled.png
          zip "$out" tiled.png colors.tdesktop-theme
        '';
      in config.lib.dag.entryAfter [ "writeBoundary" ] ''
        cp --no-preserve=mode ${theme} ${config.xdg.configHome}/telegram-base16.zip
        mv ${config.xdg.configHome}/telegram-base16.zip ${config.xdg.configHome}/telegram-base16.tdesktop-theme
      '';
    };
  };
}
