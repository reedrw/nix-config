...
with pkgs;
{
  xsession.windowManager.i3.config.keybindings = mkOptionDefault ({
    ...
    "Ctrl+Down" = "${exec} ${getExe playerctl} -a play-pause";
    "Ctrl+Left" = "${exec} ${getExe playerctl} -a previous";
    "Ctrl+Right" = "${exec} ${getExe playerctl} -a next";
    "XF86AudioPause" = "${exec} ${getExe playerctl} -a play-pause";
    "XF86AudioPlay" = "${exec} ${getExe playerctl} -a play-pause";
    "XF86AudioPrev" = "${exec} ${getExe playerctl} -a previous";
    "XF86AudioNext" = "${exec} ${getExe playerctl} -a next";
