---
description: Refresh instruction files after repo changes
---

Scan changes made to this repo since `.claude/CLAUDE.md` was last updated, and update it if anything notable warrants documentation.
...
Also skim the other instruction files (`.claude/commands/*.md`, `.claude/skills/*/SKILL.md`, `.opencode/commands/*.md`, `.opencode/skills/*/SKILL.md`) for facts invalidated by the same diff — e.g. host/target lists, commit scopes, referenced script paths. Note that `.claude/` and `.opencode/` contain copies of the same commands/skills; keep them in sync by applying any edits to both.
